...
class Entity (Module):
    def __init__(self, platform):
        ...
        # Sampling Selectors
        self.sample0 = platform.request("user_sw", 2)
        self.sample1 = platform.request("user_sw", 1)
        self.sampleFreqSel = Signal(2)
        self.comb += [
            self.sampleFreqSel[0].eq(self.sample0),
            self.sampleFreqSel[1].eq(self.sample1)
        ]
        # USER LED INDICATORS
        self.led = []
        for i in range(8):
        ...
        # SAMPLING TIME DEFINITION (ns)
        # Sampling0 Not Defined Since Its The Enable Alwasy High
        # Under Sampler 1 (self.sampleFreqSel == 1) (Divided By 4)
        Sampling1 = 100 #(10 MHz)
        # Under Sampler 1 (self.sampleFreqSel == 2) (Divided By 8)
        Sampling2 = 200 #(5 MHz)
        # Under Sampler 1 (self.sampleFreqSel == 3) (Divided By 40)
        Sampling3 = 1000 #(1 MHz)
        # Sampling Counters
        # Counter 1
        CounterAux1 = Sampling1 // WrClk
        Counter1 = CounterAux1 - 1
        # Counter 2
        CounterAux2 = Sampling2 // WrClk
        Counter2 = CounterAux2 - 1
        # Counter 3
        CounterAux3 = Sampling3 // WrClk
        Counter3 = CounterAux3 - 1
        ...
        self.sendWriteCounters = Signal()
        self.auxUartDataTxData = Signal(8)
        self.auxUartWriteTxData = Signal(8)
        self.frequencyWriteCounterEnable = Signal()
        self.secondByteCounter = Signal(8)
        self.firstByteCounter = Signal(8)
        self.writeData = Signal(11)
        self.startSendingWriteCounter = Signal()
        self.writeCounterEnable = Signal()
        # Read Enable Signals
        ...
        self.wrBigCounter1 = Signal(max=CounterAux1) # First Sampling Freq
        self.wrBigCounter2 = Signal(max=CounterAux2) # Second Sampling Freq
        self.wrBigCounter3 = Signal(max=CounterAux3) # Third Sampling Freq
        self.frequencyWrEnable = Signal()
        self.frequencyWrEnableUnder = Signal()
        self.frequencyWrEnableMilisecs = Signal()
        self.frequencyWrEnableSample1 = Signal() # First Sampling Enable Signal
        self.frequencyWrEnableSample2 = Signal() # First Sampling Enable Signal
        self.frequencyWrEnableSample3 = Signal() # First Sampling Enable Signal
        ...
        self.sync.sys180 += [
            If(~(self.samplingMode),
                ...
                If(self.sendWriteCounters,
                    If(~self.startSendingWriteCounter,
                        self.startSendingWriteCounter.eq(1),
                        self.writeCounterEnable.eq(1)
                    ),
                    If(self.changeMemory, 
                        If(self.memoryAdressed == 7,
                            self.memoryAdressed.eq(0),
                            If(self.memory[0].multiFIFOEmpty,
                                self.sendWriteCounters.eq(0),
                                self.startSendingWriteCounter.eq(0)
                            )
                        ).Else(
                            self.memoryAdressed.eq(self.memoryAdressed + 1)
                        )
                    ),
                    If(((self.lastMemoryAdressed != self.memoryAdressed) | self.writeCounterEnable), 
                        self.lastMemoryAdressed.eq(self.memoryAdressed),
                        self.writeCounterEnable.eq(0),
                        self.frequencyWriteCounterEnable.eq(1)
                    ).Else(
                        self.frequencyWriteCounterEnable.eq(0)
                    )
                )
            )                         
        ]
        ...
        self.sync.invBufrFrameClk += [
            ...
            If(self.allignmentFinished,
                ...
                If(self.wrBigCounter == Counter,
                    ...
                    self.frequencyWrEnableMilisecs.eq(1)
                ).Else(
                    self.frequencyWrEnableMilisecs.eq(0)
                ),
                # Sampling At 10 MHz
                self.wrBigCounter1.eq(self.wrBigCounter1 + 1),
                If(self.wrBigCounter1 == Counter1,
                    self.wrBigCounter1.eq(0),
                    self.frequencyWrEnableSample1.eq(1)
                ).Else(
                    self.frequencyWrEnableSample1.eq(0)
                ),
                # Sampling At 5 MHz
                self.wrBigCounter2.eq(self.wrBigCounter2 + 1),
                If(self.wrBigCounter2 == Counter2,
                    self.wrBigCounter2.eq(0),
                    self.frequencyWrEnableSample2.eq(1)
                ).Else(
                    self.frequencyWrEnableSample2.eq(0)
                ),
                # Sampling At 1 MHz
                self.wrBigCounter3.eq(self.wrBigCounter3 + 1),
                If(self.wrBigCounter3 == Counter3,
                    self.wrBigCounter3.eq(0),
                    self.frequencyWrEnableSample3.eq(1)
                ).Else(
                    self.frequencyWrEnableSample3.eq(0)
                )
            )
        ]
        # Write Enable MUX (Select Sampling Frequency With User Switches 1-2)
        self.comb += [
            Case(self.sampleFreqSel, {
                # Enables According To Sampling Frequency Selected
                0: self.frequencyWrEnableUnder.eq(1), # If There Is No Undersampler, Then System Always Writes With 40 MHz Clock
                1: self.frequencyWrEnableUnder.eq(self.frequencyWrEnableSample1), # 10 MHz Sample Frequency
                2: self.frequencyWrEnableUnder.eq(self.frequencyWrEnableSample2), # 5 MHz Sample Frequency
                3: self.frequencyWrEnableUnder.eq(self.frequencyWrEnableSample3), # 1 MHz Sample Frequency
                "default": self.frequencyWrEnableUnder.eq(1)
            }),
            # Write Frequency Enable Selector
            Case(self.samplingMode, {
                # Changes Between Sampling Modes
                0: self.frequencyWrEnable.eq(self.frequencyWrEnableMilisecs),
                1: self.frequencyWrEnable.eq(self.frequencyWrEnableUnder),
                "default": self.frequencyWrEnable.eq(self.frequencyWrEnableMilisecs)
            })
        ]
        ...
        self.comb += [
            ...
            # Write Counter Sending
            Case(self.memoryAdressed, {
                0: self.writeData.eq(self.memory[0].sysWriteCount),                
                1: self.writeData.eq(self.memory[1].sysWriteCount),
                2: self.writeData.eq(self.memory[2].sysWriteCount),
                3: self.writeData.eq(self.memory[3].sysWriteCount),
                4: self.writeData.eq(self.memory[4].sysWriteCount),
                5: self.writeData.eq(self.memory[5].sysWriteCount),
                6: self.writeData.eq(self.memory[6].sysWriteCount),
                7: self.writeData.eq(self.memory[7].sysWriteCount),
                "default": self.writeData.eq(self.memory[0].sysWriteCount)
            }),
            ...
            Case(self.byteCounter, {
                0: self.auxUartDataTxData.eq(self.firstByte),
                1: self.auxUartDataTxData.eq(self.secondByte),
                "default": self.auxUartDataTxData.eq(self.firstByte)
            }),
            # Write Data Assignation
            Case(self.byteCounter, {
                0: self.auxUartWriteTxData.eq(self.firstByteCounter),
                1: self.auxUartWriteTxData.eq(self.secondByteCounter),
                "default": self.auxUartWriteTxData.eq(self.firstByteCounter)
            })
        ]
        ...
        # Creating The Two Write Counter Bytes That Are Sent
        # First Byte
        for i in range(8):
            self.comb += self.firstByteCounter[i].eq(self.writeData[i])
        # Second Byte
        for i in range(3):
            self.comb += self.secondByteCounter[i].eq(self.writeData[i+8]) 

        # Serial Data To Send
        self.comb += [
            Case(self.sendWriteCounters, {
                0: self.uart.txData.eq(self.auxUartDataTxData),
                1: self.uart.txData.eq(self.auxUartWriteTxData),
                "default": self.uart.txData.eq(self.auxUartDataTxData)
            })
        ] 
        ...
        self.sync += [
            ...
            If((self.rdEnableFromLogic | self.frequencyWriteCounterEnable),
